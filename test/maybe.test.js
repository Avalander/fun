...
const { Maybe, Just, Nothing, fromNullable, fromList } = require('maybe')
...
test('fromList should return Just when called with non-empty array.', t => {
	t.plan(1)

	const actual = fromList([ 'Twilight Sparkle', 'Rainbow Dash' ])
	t.equal(actual.inspect(), 'Just(Twilight Sparkle,Rainbow Dash)')
})

test('fromList should return Nothing when called with empty array.', t => {
	t.plan(1)

	const actual = fromList([])
	t.equal(actual.inspect(), 'Nothing()')
})